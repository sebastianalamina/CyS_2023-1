...
public class Hill {
	...
	* @param secret Mensaje a descifrar.
	...
	*/
	public static String desencripta(String secret, String key) throws Exception {
		...
		for (int x = 0; x < n; x++)
			for (int y = 0; y < n; y++)
				llaveMInversa[x][y] %= 27;
		//System.out.print("LLAVE INVERSA CON MÓDULO: "+Arrays.deepToString(llaveMInversa));

		// Normalizamos el texto a encriptar y lanzamos error si éste no puede dividirse en N-gramas.
		String texto = Auxiliar.normalizarTexto(secret);
		//System.out.println("TEXTO NORMALIZADO: "+texto);
		if (texto.length() % n != 0)
			throw new Exception("El texto debería poder dividirse en diagramas o trigramas.");

		// Creamos una variable para ir almacenando los N-gramas
		// y otra variable para ir almacenando el texto desencriptado.
		int[][] nGrama = new int[n][1];
		...
		for (int i = 0; i < texto.length(); i++) {
			...
			if ((i+1)%n == 0) {
				...
				// ...creamos el criptograma...
				int[][] criptograma = Auxiliar.multiplicarArreglos(llaveMInversa, nGrama);

				// ...le aplicamos módulo...
				for (int j = 0; j < criptograma.length; j++)
					criptograma[j][0] %= 27;

				// ...y pegamos los caracteres correspondientes al texto desencriptado...
				for (int j = 0; j < criptograma.length; j++)
					textoDesencriptado += alfabeto[criptograma[j][0]];
			}
		}

		// Finalmente, devolvemos el texto desencriptado.
